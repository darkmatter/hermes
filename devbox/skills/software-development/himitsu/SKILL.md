...
## 1Password Service-Account Bridge

Some application credentials live in 1Password while the non-interactive `op` service-account token lives in Himitsu. Use `references/onepassword-bridge.md` to retrieve the service token without leakage, discover accessible vaults, and read the final credential in-process. Do not assume a Mac-local `Private` vault path is accessible to the service account.
