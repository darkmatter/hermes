...
2. **In-repo (this skill is about this case):** `~/hermes-agent/skills/<category>/<name>/SKILL.md` — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.

## When to Use

- User asks you to add a skill "in this branch / repo / commit"
- You're committing a reusable workflow that should ship with hermes-agent
- You're editing an existing skill under `~/hermes-agent/skills/` (use `patch` for small edits, `write_file` for rewrites; `skill_manage` still works for patch on in-repo skills, but not for `create`)
...
4. **Validate locally.** Use the repository/Hermes virtualenv interpreter that already provides PyYAML; a host system Python may not include `yaml`. Do not install PyYAML globally just for this check. If the project environment is unavailable, use Hermes's built-in skill loader/validator or a dependency-free parser for the required top-level fields.
