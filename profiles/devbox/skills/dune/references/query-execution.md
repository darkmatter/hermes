...
**json:** Full API response object. Result rows are nested under `result.rows`:
...
```json
{
  ...
  "result": {
    "metadata": { "column_names": ["col_a", "col_b"], "row_count": 2 },
    "rows": [
...
**Important:** When programmatically parsing JSON output, extract rows from `data["result"]["rows"]`, NOT `data["rows"]`.
