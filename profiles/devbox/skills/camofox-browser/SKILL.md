...
| [references/devbox-vnc-r2-passkeys.md](references/devbox-vnc-r2-passkeys.md) | Remote devbox setup with R2 persistence, VNC/noVNC, direct clipboard sync, 1Password/passkey caveats, and Chrome-to-Camofox session transfer |
| [references/remote-login-persistence.md](references/remote-login-persistence.md) | R2-backed profiles, Chrome session export, VNC clipboard workarounds, and passkey/1Password patterns |
...
**Remote human login / passkeys / clipboard?**
When a human needs to log into a remote Camofox session, use the devbox workflow in [references/devbox-vnc-r2-passkeys.md](references/devbox-vnc-r2-passkeys.md). Key lessons: mount profiles on R2 with rclone `vfs-cache-mode full`, do not rely on VNC/noVNC for reliable clipboard or USB/WebAuthn, sync clipboard directly with X11 (`xsel`/`xclip`), and prefer 1Password browser extension or a local CDP browser for passkeys.
