...
**Binary:** `gog` (Homebrew) · **Config:** `~/.config/gogcli/config.json` · **Credentials/keyring:** `~/.local/share/gogcli/` + macOS Keychain or file backend
...
### Corrupted tokens / AES unwrap failures

Error shape: `read token: ... aes.KeyUnwrap(): integrity check failed`.
...
**Config-forced file backend:** `~/.config/gogcli/config.json` may set `"keyring_backend": "file"` with no env var. Password is himitsu `gog/keyring-password` — export it before every gog command in non-interactive shells.

**Fix order:** (1) supply password from himitsu, (2) stop forcing the wrong backend, (3) re-auth into the forced backend from the same environment.
