...
**Forward slashes work almost everywhere.** `C:~ is accepted by
...
Cooper migrated from OpenClaw to Hermes. For the full migration reference
(path mappings, secret substitutions, cron schedule pitfalls, agent mapping)
see `references/openclaw-migration.md`. For the complete cron job inventory
(11 jobs, 7 still active/paused) see `references/openclaw-cron-jobs.md`.

## Composio SDK Script Pitfalls

When executing stand-alone TypeScript scripts with `@composio/core`, see
`references/composio-script-pitfalls.md` for directory context, installation,
and module type requirements.
