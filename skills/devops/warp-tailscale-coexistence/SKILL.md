...
metadata:
  hermes:
    ...
    category: devops
    related_skills: [operator-status]
---
...
| Dashboard API 401 via tunnel (HTML loads) | Tunnel `httpHostHeader` includes port (`localhost:9119`); Hermes auth gate does exact-match on Host — set to `localhost` only |
...
- Sibling: `operator-status` Mode C (Access app WARP flag + feed.cm.xyz routes)
