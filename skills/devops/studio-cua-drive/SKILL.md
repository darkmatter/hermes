...
description: >-
  Drive Mac Studio desktop/Chrome from Hermes via SSH + Studio cua-driver (never Pro
  computer_use on Screen Sharing). Umbrella for AX element-index drive, CDP vision/box_2d
  form loops, browser_prepare payments/checkouts, MCP SSH lifecycle, and local Studio Hermes.
  Absorbs studio-cua-driver / studio-cua-vision-drive / studio-browser-drive.
version: 1.2.1
---
...
- **Session-revival ladder (verified):** when a session ends, calls fail in stages — fix each stage in order, don't skip ahead: (1) "session has ended" → `start_session` with the SAME session id (returns `revived: true`); (2) reusing the old `tab_id`/`target_id` → "target is not a live binding … re-run with pid + window_id" → drop the stale ids entirely; (3) `get_browser_state(pid, window_id)` → "browser_requires_setup — run browser_prepare" → call `browser_prepare(pid, window_id, strategy.kind=existing_profile)`; (4) re-bind with `get_browser_state(pid, window_id)` — it returns NEW `target_id`/`tab_id`s (the old ones never come back); use those for all subsequent clicks. If Chrome's PID changed (restart), re-discover via `list_windows` and confirm with `ps` first.
...
## Modes (umbrella — absorbed siblings)

This skill is the **class-level Studio CUA** entry. Load it for any Mac Studio desktop/Chrome automation from Hermes.

| Mode | When | Detail |
|---|---|---|
| **A. SSH + cua-driver (AX / element index)** | Default desktop + form fill when AX is rich enough | Canonical workflow above · `references/studio-cua-runbook.md` · `references/absorbed-studio-cua-driver.md` |
| **B. Vision drive loop** | Form-heavy pages where per-field AX ping-pong is too slow | `references/vision-drive-loop.md` — CDP screenshot → one `vmap.py` sweep → box_2d 0–1000 · scripts in `scripts/vmap.py` + `scripts/vcoord.py` (also under kernel-browsers) |
| **C. Browser drive (semantic + vision)** | Payments/checkouts on logged-in Studio Chrome from `studio` profile | `references/browser-drive-loop.md` — `get_browser_state` + semantic refs + CSS-pixel actions |
| **D. Local Studio Hermes handoff** | Prefer Hermes-on-Studio over Pro→SSH bridge | `references/local-hermes-studio-handoff.md` |
| **E. MCP SSH lifecycle** | Pro Hermes MCP → Studio cua-driver stdio | `references/mcp-ssh-browser-lifecycle.md` |

**Hard rules shared by all modes:** never Pro `computer_use` on Screen Sharing; never Kernel as default (fallback only — `references/when-to-prefer-kernel.md`); stop on re-auth/MFA/payment-submit gates per `financial-operations` §6; CDP screenshots when TCC Screen Recording is false.

## Flue desktop agent (productized loop)

The same Studio MCP-over-SSH path is packaged as `@agents/desktop` + `@repo/desktop-app` in `~/git/darkmatter/agents`. Runtime identity is `desktop` (“Computer User” is leftover naming). Map, pitfalls, and review checklist: `references/flue-desktop-agent.md`.

**Do not assume code enforces the charge gate.** `cua_click` will click Pay if the model asks; the stop is prompt-only. `/agents/desktop` has no bearer <REDACTED> Do **not** add a general `local()` sandbox so the agent can run `himitsu`/`op` — that is a host shell, not a secret tool. Allowlisted secret-read tools only.

## Related
- Operator runbook: `references/studio-cua-runbook.md` (+ driver variant `studio-cua-runbook-driver.md`)
- AA canceled-PNR / travel-credit: `references/aa-online-credit-rebook.md`
- AA search/fare/passenger forms: `references/aa-booking-form-pitfalls.md` · `references/aa-booking-lookup.md`
- Payment browser verification: `references/payment-browser-verification.md`
- Flue `@agents/desktop` map + safety: `references/flue-desktop-agent.md`
- CFTC portal: `references/cftc-portal-handoff.md`
- HITL / phone stop rules: `vapi-phone-ops`
- BlueBubbles send identity: Studio BB only `cooperton42391@gmail.com` — never Pro Messages self-loop
- Payment charge gate: `financial-operations` §6
