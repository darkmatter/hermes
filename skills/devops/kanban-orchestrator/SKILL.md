...
## CLI / board operations (when you are not a dispatched worker)

Deep orchestration below assumes kanban tools. For operator-side board
management from a terminal session (create boards, park human-action cards
unassigned, bulk-create, triage-column tricks), see:

- `references/hermes-kanban-cli.md` — CLI quick reference, human-vs-agent
  parking rules, triage column DB recovery, and pitfalls (no `cards` table;
  `--initial-status blocked` does not stick on unassigned creates).
- `scripts/bulk_create_cards.py` — helper for dumping many unassigned cards.

Do **not** confuse Hermes kanban with `bd`/beads (`beads-setup`) or the
issue-tracker `triage` skill — those are separate systems.
