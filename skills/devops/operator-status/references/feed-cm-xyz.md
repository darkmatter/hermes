...
- `GET /api/meta` — snapshot age + ingest log + `pending_responses`
- `GET /api/feed` — full feed-data payload (Access-gated)
- `POST /api/ingest` — Access headers **and** Bearer <REDACTED> (cron/agent push)
- `POST /api/responses` — Cooper UI decision → D1 `feed_responses` (Access session cookie/JWT)
- `GET /api/responses?status=pending` — Hermes poll queue (Bearer <REDACTED> token + Access service token)
- `POST /api/responses/:id/claim|done|cancel` — lifecycle (Bearer)

Poll helper:

```bash
cd ~/git/darkmatter/feed
bun scripts/poll-responses.ts --status pending
bun scripts/poll-responses.ts --claim
bun scripts/poll-responses.ts --done 12
```

## HITL (primary surface)

feed.cm.xyz is a **human-in-the-loop** board — not a kanban rebuild.

| Direction | Table | Use |
|---|---|---|
| Agent → Cooper | `hitl_requests` | Any decision: pay/approve/pick/confirm |
| Cooper → Agent work | `feed_responses` | After answer, or “do this” with progress bar |

```bash
# Agent asks Cooper
bun ~/git/darkmatter/feed/scripts/hitl.ts ask \
  --title "Pay Kernel \$30?" \
  --body "Investigated: card ••1234 declined; repair ready." \
  --option "*pay:✅ Pay" --option "skip:❌ Don't pay" \
  --category pay --priority 80

bun scripts/hitl.ts list --status open
bun scripts/hitl.ts wait 12   # block until answered
```

Cooper opens https://feed.cm.xyz → **Needs your decision** → Submit answer.
Agent activity panel shows execution progress separately.

After **Send to Hermes**, UI shows live progress from D1:

`queued (0%) → claimed (~10%) → running (message + pct) → done/failed (100%)`

| Agent call | Effect |
|---|---|
| `POST /api/responses/:id/claim` | claimed + “starting work” |
| `POST /api/responses/:id/progress` `{message, progress_pct}` | running + bar update |
| `POST /api/responses/:id/done` `{result_summary}` | done 100% |
| `POST /api/responses/:id/fail` `{error}` | failed |

```bash
bun scripts/poll-responses.ts --status open --claim
bun scripts/poll-responses.ts --progress 12 --message "Checked Stripe portal" --pct 40
bun scripts/poll-responses.ts --done 12 --summary "Kernel $30 repaired; receipt …"
```

Dashboard **Agent activity** section auto-polls open work every few seconds.
