...
description: >
  ...
  production feed.cm.xyz / D1 ingest), blocked-kanban + digests + recommendations,
  and human-readable weekly review (accomplished / still-open / next-week). Use for
  feed rebuilds, feed.cm.xyz push/Access, dashboard layout, weekly status reports,
  "what happened this week", or cron wiring. Supersedes daily-feed, weekly-review,
  and cooper-feed-dashboard as separate skills.
version: 1.0.0
metadata:
  hermes:
...
# Mode C — Production feed.cm.xyz (Waku + CF Worker + D1)

Prefer **https://feed.cm.xyz** over localhost when Cooper asks about “the dashboard.”
Local `~/feed` + launchd `:8654` remains the bootstrap/build host; production freshness
= last successful **ingest**, not only last local rebuild.

## Architecture (prod)

```
build-feed.py / Daily Feed Builder cron
    → ~/feed/src/feed-data.json
    → bun scripts/push-feed.ts   (Bearer <REDACTED> + CF Access headers)
...
| Surface | Path | URL |
|---|---|---|
| **Production** | `~/git/darkmatter/feed` | **https://feed.cm.xyz** |
| Local legacy | `~/feed` + launchd | http://localhost:8654 |
| workers.dev | same Worker | https://cooper-feed.drtkmttr.workers.dev |

### Auth

- **Browser:** Cloudflare Access app `feed` on `feed.cm.xyz` (team **`drkmttr`.cloudflareaccess.com**).
- **Agent ingest:** `Authorization: Bearer` + `FEED_INGEST_TOKEN` **and** `CF-Access-Client-Id/Secret` (himitsu paths `feed/ingest-token`, `cf-access-client-id`, `cf-access-client-secret`).
- **Access API:** wrangler OAuth often lacks Access scopes — use himitsu `cloudflare-global-api-token` + email. Full recipe: `references/cloudflare-access-gating.md`.

### Quickops

```bash
curl -sS https://feed.cm.xyz/api/healthz
# Load FEED_INGEST_TOKEN and CF Access client id/secret via himitsu (two-step redirect — never inline into shell history)
bun ~/git/darkmatter/feed/scripts/push-feed.ts --from ~/feed/src/feed-data.json --url https://feed.cm.xyz
# or rebuild+push (build-feed.py falls back to himitsu when env lacks FEED_INGEST_TOKEN):
python3 ~/.hermes/scripts/build-feed.py
```

Header badge = data snapshot age (`generated_at` from D1), not UI deploy time.

### Prod pitfalls (summary)

| Symptom | Fix |
|---|---|
| `Schema.Defect is not a function` | Pin effect@4.0.0-beta.102 |
| Deploy OK but feed.cm.xyz CF 1104 | zone route `feed.cm.xyz/*` → `cooper-feed` |
| workers.dev 1042 on `*.darkmatter.workers.dev` | account subdomain is **`drtkmttr`** |
| Transparent sidebar / overlay content | sidebar CSS tokens + `SidebarInset` — `references/ui-ingest-and-routes.md` |
| Manual stub cron-json empties section | match full schema (`cron-json-schema.md`); re-run build-feed |
| WARP Access skip OTP | do **not** re-enable without fixing WARP+Tailscale DNS (`warp-tailscale-coexistence`) |

Full pre-merge body + IDs: `references/absorbed-cooper-feed-dashboard.md`, `references/feed-cm-xyz.md`.

Hermes public dashboard (hermes.cm.xyz) is a **different** surface — use **`hermes-dashboard-ops`** (also has `references/hermes-cm-xyz-dashboard.md`).
...
| `references/daily-feed-skill.md` | Full pre-merge daily-feed body |
| `references/weekly-review-skill.md` | Full pre-merge weekly-review body |
| `references/feed-cm-xyz.md` | prod IDs, Access app, deploy anchors |
| `references/cloudflare-access-gating.md` | Access service-token + IP gating |
| `references/ui-ingest-and-routes.md` | ResponseComposer, SidebarInset, routes |
| `references/absorbed-cooper-feed-dashboard.md` | Full pre-merge prod feed skill |
...
- `alchemy` — org Alchemy deploy patterns for the Worker
- `hermes-dashboard-ops` — hermes.cm.xyz (not feed.cm.xyz)
- `warp-tailscale-coexistence` — dual-tunnel DNS before re-enabling WARP Access
