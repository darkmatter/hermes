...
For travel booking, checkout, and other payment-bearing flows, also follow [`references/transaction-safe-shopping.md`](references/transaction-safe-shopping.md): separate inspection, fare selection, final-price review, authorization, submission, and post-submit verification.
...
## AA reservation lookup / upgrade checks (read-only)
For an existing reservation, AA's `Find your trip` flow requires **last name + confirmation/ticket + DOB**; locator/name alone triggers `Please enter a valid date.` Use the stable `name=` fields and **exact accessible name** `Find your trip` (AA duplicates `id="button"` for cookie dismiss + submit). Report an upgrade only if AA exposes a concrete upgrade/cabin offer or action; `Change trip`, `Change seats`, bags, and ancillaries are not upgrade availability. Full method and the custom DOB select values: **`references/aa-manage-reservation.md`**.
