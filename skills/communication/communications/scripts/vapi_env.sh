...
#
# 1Password: service-account ONLY via ~/.local/bin/op + --vault.
# Never bare brew/nix op (biometric prompt). Never agenix personal token.
set -euo pipefail

# Prefer SA op wrapper before any real op on PATH
...
resolve_private_key() {
  ...
  # himitsu first when present
  if command -v himitsu >/dev/null 2>&1; then
    local hk
    hk="$(himitsu read vapi/private-key 2>/dev/null || himitsu read vapi/api-key 2>/dev/null || true)"
    if [[ -n "$hk" ]]; then
      printf '%s' "$hk"
      return 0
    fi
  ...
  # 1Password SA item "vapi" in vault dev (drkmttr) — wrapper injects token
  if command -v op >/dev/null 2>&1; then
    local k
    k="$(OP_TIMEOUT="${OP_TIMEOUT:-8}" op item get vapi --vault dev --fields credential --reveal 2>/dev/null || true)"
    if [[ -n "$k" ]]; then
      printf '%s' "$k"
      return 0
    fi
...
PY
  ...
  echo "vapi_env: no private key found. Set VAPI_API_KEY, or ensure SA op can read --vault dev item vapi (no biometric op)." >&2
  return 1
}
