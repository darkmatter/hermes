...
name: vapi-phone-ops
description: >-
  Operate Cooper's Vapi phone stack (umbrella) — outbound AI calls (Levi Okada), live control,
  warm-transfer-experimental with Levi fallback, DTMF/IVR, mid-call HITL (ask_cooper/text_cooper),
  BB iMessage status (not Twilio SMS), analytics/CLI, payment-on-call, hangup/runaway kill.
  Use for ANY Vapi call work. Absorbs vapi-call-ops / vapi-phone / vapi-phone-calls / ai-phone-calls.
version: 3.3.3
metadata:
  hermes:
    tags: [vapi, phone, telephony, analytics, transfer, twilio, elevenlabs, hitl, dtmf]
    category: communication
    related_skills: [communications, bluebubbles-studio, studio-cua-drive]
---
...
- **communications** — multi-channel map + shared scripts/templates (`vapi_env.sh`, `watch_call.sh`, `vapi_analytics.sh`, `ask_cooper_server.py`, call JSON templates)
- **financial-operations** — 1P service-token pattern for card pull

## Absorbed sibling coverage (this is the Vapi umbrella)

Consolidated from `vapi-call-ops`, `vapi-phone`, `vapi-phone-calls`, and `ai-phone-calls`. Prefer **this skill** for any Vapi call work; do not load the archived stubs.

| Topic | Primary section / support file |
|---|---|
| Outbound SOP + hold-proof overrides | Outbound call workflow (above) |
| Warm-transfer-experimental + Levi fallback | Transfers · `references/warm-transfer-and-hitl.md` · `references/warm-transfer-sop.md` |
| DTMF vs keypadInput vs SIP INFO | IVR / DTMF · `references/ivr-and-channels.md` |
| HITL ask_cooper / text_cooper | HITL · Tool usage · `references/knowledge-base-and-hitl.md` |
| Payment-on-call + hangup/runaway | Payment · `references/payment-and-hangup.md` · `references/kill-and-ops.md` |
| Call outcome discipline (success early / fail only after log) | Call outcome section · `references/call-outcome-*.md` · `references/digit-and-outcome-discipline.md` |
| Analytics + CLI | Analytics + CLI · `references/analytics-cli.md` · `references/vapi-api.md` |
| BB status / A2P 30034 | Messaging · `references/bluebubbles-status.md` · `references/bluebubbles-studio-webhook.md` · `references/nix-bb-durable.md` |
| Airline rebook lessons (AA WSZTVR etc.) | `references/airline-rebook-lessons.md` · `references/ops-checklist.md` |
| Pre-merge full bodies | `references/absorbed-vapi-*.md`, `references/absorbed-ai-phone-calls.md` |
| Extra templates (from ai-phone-calls) | `templates/outbound_call.json`, `templates/flight_change_call.json` |

Shared runtime scripts still live under `communications/scripts/` (canonical). Optional copies under this skill’s `scripts/` are labeled `-ai-phone` legacy variants only.
