...
## Step 2: Get 1Password Credentials (service account only)

```bash
export PATH="$HOME/.local/bin:$PATH"   # SA wrapper — never bare brew/nix op
# Catalog first: ~/.hermes/op-sa-catalog.json (names only)
# BofA login titles in vault cm: secure.bankofamerica.com, BofA Personal Debit (card)

op item get secure.bankofamerica.com --vault cm --fields username --format json
op item get secure.bankofamerica.com --vault cm --fields password --reveal --format json
```

Never call `op` without `--vault`. Never use interactive/biometric unlock.
